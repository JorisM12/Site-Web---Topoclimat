...
btnSearch.addEventListener('click',()=>{
    deleteWeatherCard();
    stopButton(btnSearch);
    errorElem.style.display = 'none';
    const coords = cityCoordsInput.value;
    const coordsArray = coords.split(',');
    const t = city(coordsArray[1],coordsArray[0],cityInput.value);
    t.then(response=>{
        console.log(cards);
            setTimeout(()=>{
                let index = '1';
...
async function startCard(start,numDay = 'J1',lat, lon,day = 0) {
    deleteWeatherCard();
...
async function city(lat, lon, city) {
    ...
    await startCard(J1, 'J1', lat, lon, todayDate('J1', true));
    await startCard(J2, 'J2', lat, lon, todayDate('J2', true));
    await startCard(J3, 'J3', lat, lon, todayDate('J3', true));
    await startCard(J4, 'J4', lat, lon, todayDate('J4', true));
    await startCard(J5, 'J5', lat, lon, todayDate('J5', true));
    await startCard(J6, 'J6', lat, lon, todayDate('J6', true));
